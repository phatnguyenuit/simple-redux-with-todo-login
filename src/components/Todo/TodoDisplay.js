...
export default class TodoDisplay extends React.Component {
	...
	handleUpdateStatus = (id, dispatch) => e => {
		...
		dispatch(
			todoAction.actionUpdateTodo(id, {status: newStatus})
		);
	}
		...
		return (
			<tr>
				...
				<td>
					...
					<div className="form-group">
						<select className="form-control" onChange={this.handleUpdateStatus(id, dispatch)} value={status}>
							{
								statusOptions.map(({ name, value }) => {
									const disabled = value === 'all';
									return <option value={value} key={value} disabled={disabled}>{name}</option>
								})
							}
						</select>
					</div>
